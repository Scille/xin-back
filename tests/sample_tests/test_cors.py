...
class TestCORS(common.BaseTest):
    ...
    def test_no_origin(self):
        # No origin, no access
        r = self.client_app.options('/me')
        assert r.status_code == 200
        for cors_header in CORS_HEADERS:
            assert cors_header not in r.headers
        # Bad origin, no access
        r = self.client_app.options('/me', headers={'Origin': 'http://bad-origin.org'})
        assert r.status_code == 200
        for cors_header in CORS_HEADERS:
            assert cors_header not in r.headers
    ...
    def test_expose(self):
        ...
        r = self.client_app.options('/me', headers={'Origin': origin})
        assert r.status_code == 200
    ...
    def test_allow(self):
        ...
        r = self.client_app.options('/me', headers=headers)
        assert r.status_code == 200
